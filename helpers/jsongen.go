...
func splitSameValues(val []Values) bool {
	occured := map[string]bool{}
	result := false
	for _, e := range val {
		// check if already the mapped
		// variable is set to true or not
		if occured[e.ValueName] != true {
			occured[e.ValueName] = true
		}
...
func Treegen(dataGPO []AllPolicies) string {
	...
	for _, pol := range dataGPO {
		...
		/*if strings.ToLower(pol.Class) == "both" {
			x := "User" + "/" + pol.Category + "/"
	...
	for i := 0; i < len(dr); i++ {
		...
		for _, pol := range dataGPO {
			...
			if strings.EqualFold(sl[0], pol.Class) {
				if strings.EqualFold(splt1, pol.Category) { //&& x == pol.ID {;
					web = append(web, tmp+"	{")
					web = append(web, tmp+"\"id\": "+strconv.Itoa(pol.ID)+",")
					nm, _ := json.Marshal(pol.Name)
					dn, _ := json.Marshal(pol.DisplayName)
					pr, _ := json.Marshal(pol.Presentation)
					//vw, _ := json.Marshal(pol.View)
					web = append(web, tmp+"\"name\": "+string(nm)+",")
					...
					web = append(web, tmp+"\"presentation\": "+string(pr)+",")
					//web = append(web, tmp+"\"view\": "+string(vw)+",")
